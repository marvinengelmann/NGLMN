...
describe("detectModelCorrection", () => {
  it("returns null when previous mood is unknown", () => {
    const previous = { ...DEFAULT_OPERATOR_MODEL, estimatedMood: "unknown" as const }
    const next = { ...DEFAULT_OPERATOR_MODEL, estimatedMood: "happy" as const }
    expect(detectModelCorrection(previous, next)).toBeNull()
  })

  it("returns null when mood unchanged", () => {
    const previous = { ...DEFAULT_OPERATOR_MODEL, estimatedMood: "happy" as const }
    const next = { ...DEFAULT_OPERATOR_MODEL, estimatedMood: "happy" as const }
    expect(detectModelCorrection(previous, next)).toBeNull()
  })
