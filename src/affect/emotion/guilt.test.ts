...
import { compute, computeEffect, defaultState, markRepaired } from "./guilt.ts"
...
describe("compute", () => {
  ...
  it("triggers on missed workflow", () => {
    const result = compute(makeContext({ missedWorkflow: true }))
    expect(result.isActive).toBe(true)
...
describe("computeEffect", () => {
  ...
  it("dampens satisfaction when active", () => {
    const state = { ...defaultState, level: 0.5, isActive: true, repairMotivation: 0.6 }
    const result = computeEffect(state)
    expect(result.satisfaction).toBeLessThan(0)
  })

  it("boosts energy when repair motivation is high", () => {
    const state = { ...defaultState, level: 0.5, isActive: true, repairMotivation: 0.7 }
    const result = computeEffect(state)
    expect(result.energy).toBeGreaterThan(0)
  })

  it("boosts connection when repair motivation exists", () => {
    const state = { ...defaultState, level: 0.5, isActive: true, repairMotivation: 0.5 }
    const result = computeEffect(state)
