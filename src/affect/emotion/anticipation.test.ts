...
import { compute, computeEffect, defaultState } from "./anticipation.ts"
...
describe("compute", () => {
  ...
  it("triggers on planned activity", () => {
    const result = compute(makeContext({ plannedActivity: true }))
    expect(result.isActive).toBe(true)
  ...
  it("dampens with active disappointment", () => {
    const withDisappointment = compute(makeContext({ plannedActivity: true, disappointmentActive: true }))
    const without = compute(makeContext({ plannedActivity: true }))
  ...
  it("tracks buildup ticks", () => {
    const result = compute(
      makeContext({
        ...
        previousState: { ...defaultState, buildupTicks: 3, level: 0.3, isActive: true }
      })
    )
...
describe("computeEffect", () => {
  ...
  it("boosts excitement and energy", () => {
    const state = { ...defaultState, level: 0.5, isActive: true, valence: 0.7 }
  ...
  it("reduces boredom", () => {
    const state = { ...defaultState, level: 0.5, isActive: true, valence: 0.5 }
    const result = computeEffect(state)
    expect(result.boredom).toBeLessThan(0)
  })

  it("scales excitement with positive valence", () => {
    const lowValence = { ...defaultState, level: 0.5, isActive: true, valence: 0.2 }
    const highValence = { ...defaultState, level: 0.5, isActive: true, valence: 0.9 }
    const lowEffect = computeEffect(lowValence)
    const highEffect = computeEffect(highValence)
