...
import { compute, computeEffect, defaultState } from "./ambivalence.ts"
...
describe("compute", () => {
  ...
  it("respects max pairs limit", () => {
    ...
    const result = compute(
      makeContext({
        previousState: { ...defaultState, activePairs: manyPairs }
      })
    )
...
describe("computeEffect", () => {
  ...
  it("scales confidence drain with paralysis risk", () => {
    const lowParalysis = computeEffect({
      ...defaultState,
      level: 0.5,
      isActive: true,
      paralysisRisk: 0.1
    })
    const highParalysis = computeEffect({
      ...defaultState,
      level: 0.5,
      isActive: true,
      paralysisRisk: 0.8
