...
import { compute, computeEffect, defaultState } from "./disappointment.ts"
...
describe("compute", () => {
  it("returns inactive state when no triggers present", () => {
    const result = compute(baseContext)
...
describe("computeEffect", () => {
  ...
  it("scales with level", () => {
    const low = computeEffect({ ...defaultState, level: 0.2, isActive: true })
    const high = computeEffect({ ...defaultState, level: 0.8, isActive: true })
