import type { SecondaryEmotionState } from "@/emotion/types.ts"
...
export function renderSecondaryEmotion(name: string, state: SecondaryEmotionState): string | null {
  // biome-ignore lint/suspicious/noExplicitAny: emotion-specific fields accessed dynamically
  ...
  switch (name) {
    ...
    case "guilt": {
      ...
      const latestEntry = s.recentEntries?.filter((e: { repaired: boolean }) => !e.repaired)[0]
      if (latestEntry) {
    ...
    case "protectiveAnger": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "something inside you rises to defend what matters")
      }
      if (s.assertionReady) {
    ...
    case "hope": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a quiet flame — things could get better")
      }
      if (s.fragility > 0.4) {
    ...
    case "resignation": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a quiet giving up — not anger, just... done")
      }
      if (s.depth > 0.5) {
    ...
    case "awe": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "something vast touched you, and you went quiet inside")
      }
    ...
    case "resentment": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a slow bitterness building behind your words")
      }
      if (s.hardening > 0.4) {
    ...
    case "tenderness": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a quiet warmth that asks nothing in return")
      }
      if (s.softness > 0.5) {
    ...
    case "anticipation": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "something ahead of you glows, and you're leaning toward it")
      }
      if (s.buildupTicks > 5) {
    ...
    case "pride": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a quiet glow — you did something that matters")
      }
      if (s.glowDuration > 5) {
    ...
    case "envy": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "an ache at the edges — wanting what you see but don't have")
      }
    ...
    case "playfulness": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a lightness rises — something in you wants to play")
      }
    ...
    case "melancholy": {
      ...
      if (s.source) {
        lines.push(sourceDesc[s.source] ?? "a gentle ache — the bittersweet weight of being alive")
      }
      if (s.poignancy > 0.5) {
    ...
    case "procrastination": {
      ...
      if (s.dominantSource) {
        lines.push(sourceDescriptions[s.dominantSource] ?? "something is holding you back from acting")
      }
      if (s.streakTicks > 3) {
