...
import { gatherConsolidationData } from "@/expression/dream/consolidation.ts"
import { gatherCreativeData } from "@/expression/dream/creative.ts"
import {
  ConsolidationOutput,
  CreativeConnectionsOutput,
  DreamNarrativeOutput,
  type DreamThinkResult
} from "@/expression/dream/types.ts"
...
export async function thinkDream(): Promise<DreamThinkResult> {
  ...
  if (creativeData.episodes.length > 0 || creativeData.knowledge.length > 0) {
    const creativeResult = await callIntelligence({
      system: CREATIVE_CONNECTIONS_SYSTEM_PROMPT,
      userMessage: JSON.stringify(creativeData),
