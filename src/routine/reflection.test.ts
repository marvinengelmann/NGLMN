...
import { performReflection, runReflection, shouldTriggerReflection } from "./reflection.ts"
...
const mockSetReflectionLastAt = setReflectionLastAt as ReturnType<typeof vi.fn>
...
const mockGetActiveGoals = getActiveGoals as ReturnType<typeof vi.fn>
...
describe("runReflection", () => {
  beforeEach(() => {
    mockStoreEpisode.mockResolvedValue("ep-id")
    mockStoreKnowledge.mockResolvedValue(ok("sem-id"))
    mockSetReflectionLastAt.mockResolvedValue(undefined)
    ...
    mockGetBudgetState.mockResolvedValue(makeBudgetState())
  })

  it("builds input, performs reflection, persists, and returns result", async () => {
    mockCallIntelligence.mockResolvedValue(
      ok({
        insights: ["Insight from run"],
        personalityDeltas: null,
        newGoals: null,
        emotionalCorrections: null
      })
    ...
    const result = await runReflection("morning")

    expect(result.reason).toBe("morning")
    expect(result.output.insights).toContain("Insight from run")
    expect(mockSetReflectionLastAt).toHaveBeenCalled()
  })

  it("returns empty insights on failure instead of throwing", async () => {
    mockCallIntelligence.mockRejectedValue(new Error("LLM down"))

    const result = await runReflection("ad-hoc")

    expect(result.reason).toBe("ad-hoc")
