...
export const AnimaDecision = z.object({
  ...
  messages: z.array(
    z.object({
      ...
      voiceSegments: z.array(z.string()).optional(),
      backgroundSound: z.string().optional(),
      backgroundSoundVolume: z.number().min(0).max(1).default(0.25),
  ...
  actionPayload: z
    .object({
      ...
      lifeEventDurationHours: z
        .number()
        .min(0.1)
        .describe("Duration in hours within the allowed range for the chosen event type, e.g. 0.5 for 30 minutes")
        .optional(),
